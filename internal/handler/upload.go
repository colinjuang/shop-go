...
func (h *UploadHandler) UploadFile(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusBadRequest, model.ErrorResponse(http.StatusBadRequest, "Missing file"))
		return
	}
...
func (h *UploadHandler) BatchUploadFiles(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusBadRequest, model.ErrorResponse(http.StatusBadRequest, "Invalid form data"))
		return
	}

	files := form.File["files"]
	if len(files) == 0 {
		c.JSON(http.StatusBadRequest, model.ErrorResponse(http.StatusBadRequest, "No files provided"))
		return
	}
...
func (h *UploadHandler) DeleteFile(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, model.ErrorResponse(http.StatusBadRequest, "Invalid request"))
		return
	}
